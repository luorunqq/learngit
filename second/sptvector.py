...
def smoP(dataMatIn, classLabels, C, toler, maxIter, kTup=('lin', 0)):
    ...
    while(iter < maxIter) and ((alphaPairsChanged > 0) or (entireSet )): # 优化的终止条件：在规定迭代次数下，是否遍历了整个样本或 alpha 是否优化
        ...
        if entireSet: #
            ...
            print(nonzero((oS.alphas .A > 0) * (oS.alphas.A < C)))
...
def testRbf(k1=1.3):
    ...
    # dataArr, labelArr = make_classification(n_samples=100,n_features=25,n_classes=2,random_state=25,n_informative=24,n_redundant=0,n_repeated=0)  # 训练样本的提取
    ...
    for i in range(m):
        ...
        if sign(predict)!=sign(labelArr[i]):
            errorCount += 1
    print("the training acc rate is: %f" % (1-(float(errorCount)/m)))
    # dataArr,labelArr = make_classification(n_samples=100,n_features=25,n_classes=2,random_state=25,n_informative=24,n_redundant=0,n_repeated=0)# 测试样本集的提取
    ...
    for i in range(m):
        ...
        if sign(predict)!=sign(labelArr[i]):
            errorCount += 1
    print("the test acc rate is: %f" % (1-(float(errorCount)/m)))
